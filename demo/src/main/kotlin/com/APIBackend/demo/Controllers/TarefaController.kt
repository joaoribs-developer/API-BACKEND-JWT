...
class TarefaController(userRepository: UserRepository, val tarefaRepository: TarefaRepository): BaseController(userRepository) {
    ...
    @PutMapping("/{id}")
    fun updateTask(@RequestBody req: Tarefa,
                   @PathVariable id: Int,
                   @RequestHeader("Authorization") authorization: String):
            ResponseEntity<Any>{
        val user = readToken(authorization)
        try {
            ...
            val task = tarefaRepository.findByIdOrNull(id)
         if(task == null){
             val newTask = Tarefa(nome = req.nome, descricao = req.descricao, users = user)
